...
const Home: React.FC = () => {
  ...
  const getCloudinaryUrl = (publicId: string, width?: number, height?: number) => {
    ...
    const getCloudinaryVideoUrl = (publicId: string) => {
      const cloudName = 'dqgjdxwgw';
      return `https://res.cloudinary.com/${cloudName}/video/upload/q_auto:eco/${publicId}.mp4`;
    };
...
<div className="relative w-full min-h-screen overflow-hidden pt-24 lg:pt-28">
  {/* The browser selects one responsive source instead of downloading both videos. */}
  <div className="absolute top-0 left-0 w-full h-full">
    <video
      ...
      playsInline
      preload="metadata"
      poster={getCloudinaryUrl('events/wedding', 1600, 900)}
      className="w-full h-full object-cover"
    >
      <source
        media="(min-width: 768px)"
        src={getCloudinaryVideoUrl('indianmarriage2')}
        type="video/mp4"
      />
